...
<title>Crossword Layout Editor — 15×15</title>
<style>
:root{
...
<div class="wrap">
  <div class="header">
    ...
    <div class="controls">
      ...
      <label class="btn">
        ⬆️ Import JSON <input id="importFile" type="file" accept=".json" hidden>
      </label>
...
function onCellClick(r,c){
  if(tool==="block"){
    grid[r][c] = (grid[r][c]==="#") ? "" : "#";
    // if blocking, clear number
    if(grid[r][c]==="#") numbers[r][c]=0;
  }else{
  ...
  for(let r=0;r<rows;r++){
    for(let c=0;c<cols;c++){
      ...
      if(leftBlocked || topBlocked){
        ...
        // keep existing number only if already set manually (do nothing)
      }
    }
  }
